...
/**
  ...
  */
class MesosSchedulerDriver(eventHandler: Scheduler,
                           frameworkInfo: FrameworkInfo,
                           masterInfo: String) extends SchedulerDriver {

  private val log = Logger.get(getClass)
  private val clientFactory = new MesosMasterHttpClientFactory(frameworkInfo, this)
  private val schedulerStreamingClient: StreamingClient = if(masterInfo.startsWith("zk")) {
    ...
    new ZkAwareHttpClient(curatorFramework, zkConnectString.getPath, eventHandler, clientFactory)
  } else {
    val hostAndPort = HostAndPort.fromString(masterInfo)
    clientFactory.newClient(hostAndPort, eventHandler)
  }
